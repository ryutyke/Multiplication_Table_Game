...
private:
	...
	int money;
public:
	Student(string _name, int _age, int _life, int _exp, int _level, int _money) :name(_name), age(_age), life(_life), exp(_exp), level(_level), money(_money) {}
	...
	void fullinform() { cout << "이름: " << name << ", 나이: " << age << ", 경험치: " << exp << ", 레벨: " << level << ", 골드: " << money << endl << endl; }
...
void readStudentFile(vector<Student>& students) {
	...
	string name; int age; int life; int exp; int level; int money;
	ifstream fin("students.txt");

	fin >> name >> age >> life >> exp >> level >> money;
	while (!fin.eof()) {
		Student a = Student(name, age, life, exp, level, money);
...
void writeStudentFile(vector<Student>& students) {
	...
	for (int i = 0; i < studentsCnt; i++) {
		...
		fout << name << "\t" << age << "\t" << life << "\t" << exp << "\t" << level << "\t" << money << "\t";
		fout << endl;
	}
	fout.close();
}

...
public:
	...
	void bossAppear() {
		cout << "보스 몬스터 " << name << "이(가) 출몰했다!!!" << endl;
	}
...
void readBossFile(vector<Monster>& bosses) {
	string name; int hp; int force; int exp; int money;
	ifstream fin("bosses.txt");
...
void bossBattle(vector<Student>& students, Student& student, Monster boss) {
	string weaponSound = student.getWeaponSound();
	int num1, num2;
	int answer;
	boss.fullform();
	...
	while (true) {
		...
		if (answer == (num1 * num2)) {
			...
			Sleep(1000);
			system("cls");
			boss.damaged(num1*num2, student);
		}
		else {
			cout << "틀렸다..." << endl;
			Sleep(1000);
			system("cls");
			student.damaged(boss.getforce());
		}
		...
		if (boss.getHp() <= 0) {
			cout << "\n물리쳤다!" << endl;
			Sleep(400);
			student.increaseExp(boss.getExp());
			student.increaseMoney(boss.getMoney());
			cout << boss.getMoney() << "의 골드를 얻었다! 현재 내 골드는 " << student.getMoney() << "이다!" << endl;
			cout << boss.getExp() << "의 경험치를 얻었다! 현재 내 경험치는 " << student.getExp() << "/100 이다." << endl << endl;
			Sleep(800);
			if (student.getExp() >= 100) {
				int cnt = 0;
				int exp = student.getExp();
...
void shop(vector<Student>& students, Student& student) {
	...
	cout << "8. 체력회복(체력 + 20) 250원 " << endl;
	vector<int> price{100, 500, 1000, 2000, 3000, 4000, 6000, 250};
	int choice;
	cin >> choice;
	system("cls");
	if (choice <= 7 && choice >= 1) {
		...
		if (student.useMoney(price[choice - 1])) {
			cout << "여기요~" << endl; cout << "잔액은 " << student.getMoney() << "원 입니다." << endl << endl;
			Sleep(800);
			cout << "장착중...." << endl;
			Sleep(800);
		...
		if (student.useMoney(price[choice - 1])) {
			...
			student.heal(20);
			writeStudentFile(students);
			cout << student.getName() << "의 피가 " << student.getLife() << "/100 이 되었다." << endl;
			Sleep(800);
		}
	}
	else {
...
int main() {
	...
	while (true) {
		...
		int bossNum = rand() % bosses.size();
		Monster monster = monsters[monsterNum];
		Monster boss = bosses[bossNum];
		...
		case 2:
			...
			findBossMonster();
			boss.bossAppear();
			bossBattle(students, student, boss);
			break;
		case 3:
			system("cls");
